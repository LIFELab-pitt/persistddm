...
% =========================================================================
order          = {'Win_First','Loss_First'};
taskVersion         = 2-mod(sID,2); % odd = Win_First, even = Loss_First
blocksequences{1}   = [25 25 25 25 25 25 25 25 25 25 25 25]; % length needs to be even number! 
blocksequences{2}   = [25 25 25 25 25 25 25 25 25 25 25 25]; % length needs to be even number! 
blocks              = blocksequences{taskVersion};

prep.sID            = sID;
prep.order     = order{taskVersion};
prep.prob           = 80/100; % feedback contingency
prep.nStim          = 2;
prep.nt             = sum(blocks);
...
for i =1:6
    flipped_coin=round(rand(1,1));
    first_index=i*2-1;
    second_index=i*2;
    if flipped_coin==1
        probTrial(first_index)=prep.prob;
        probTrial(second_index)=1-prep.prob;
    elseif flipped_coin==0
        probTrial(first_index)= 1-prep.prob;
        probTrial(second_index)= prep.prob;
    end
end
...
if runTask
    ...
    prep.cue{1} = fullfile(prep.dir.imgs,'Win_slot1_up.jpg'); % blue
    prep.cue{2} = fullfile(prep.dir.imgs,'Win_slot2_up.jpg'); % orange
    prep.cue{3} = fullfile(prep.dir.imgs,'Win_slot1_down.jpg'); % blue
    prep.cue{4} = fullfile(prep.dir.imgs,'Win_slot2_down.jpg'); % orange
    prep.cue{5} = fullfile(prep.dir.imgs,'Lose_slot1_up.jpg'); % yellow
    prep.cue{6} = fullfile(prep.dir.imgs,'Lose_slot2_up.jpg'); % purple
    prep.cue{7} = fullfile(prep.dir.imgs,'Lose_slot1_down.jpg'); % yellow
    prep.cue{8} = fullfile(prep.dir.imgs,'Lose_slot2_down.jpg'); % purple
    
    % Drawing size parameters
    prep.draw.fx        = 7; % radius of the fixation cross
    prep.stimDistance   = wdh*.75; %Changed value to .75 previously .2; OR 175 distance of the stimuli from the centre
    prep.draw.rStim     = round(wdh/7); %Changes size of slots OR 85
    prep.draw.rSmile    = round(wdh/17);%Changes size of Coin OR 45
