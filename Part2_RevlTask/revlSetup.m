...
subTag      = sprintf('RevLearn_%s_s%03.0f',prep.order,sID);
...
tmp.feedback{3}    = importdata(fullfile(prep.dir.imgs, 'red.jpg')); %loss feedback
...
img.feedback{3}   = Screen('MakeTexture', wd, tmp.feedback{3});
...
for k = 3:4
    img.stim{k}   = Screen('MakeTexture',wd,tmp.cue{k+2});
    img.select{k}      = Screen('MakeTexture',wd,tmp.cue{k+4});
end
